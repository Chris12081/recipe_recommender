...
###
try:
    with open('./hiyd/url_list.txt', 'a+', encoding='utf-8') as file:
        file.close()
except:
    pass
...
def savemongodb(value):
    insert_item = value
    insert_result = db.hiyd.insert_one(insert_item)
    print(insert_result)
...
def getArticle(page):
    ...
    for times in range(page):  #pages
        ...
        for t in title:
            ...
            # print(title_url)
            with open('./hiyd/url_list.txt', 'r', encoding='utf-8') as f:
                file = f.readlines()
                url_list = []
                for i in file:
                    result = i.replace('\n', '')
                ...
                if title_url in url_list:
                    ...
                    for d in range(daily_count):
                        try:
                            ...
                            daily_title_4_1 = str.convert(daily_title_4[3].text) + ':'+ str.convert(daily_title_4[4].text).replace('.','_')
                    ...
                    value = {}
                    value['url'] = title_url
                    value['title'] = title_name_1
                    value['lesson'] = daily_a
                    value['lesson_time'] = times
                    value['strengh'] = strengh[1]
                    value['describe'] = describe_1
                    value['time'] = 0
                    value['author'] = 0
                    savemongodb(value)
            with open('./hiyd/url_list.txt', 'a', encoding='utf-8') as u:
                out_str = title_url + '\n'
                u.write(out_str)
